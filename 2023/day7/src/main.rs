...
fn get_hand_type(hand: &str) -> EHandType {
    ...
    let joker_count = *freq_map.entry('J').or_insert(0);
    ...
    let has_full_house = has_three_kind && freq_map.values().any(|&value| value == 2); 
    let has_two_pair = freq_map.values().filter(|&&value| value == 2).count() == 2; 
    let has_one_pair = freq_map.values().any(|&value| value == 2); 


    if has_five_kind || (has_four_kind && joker_count >= 1) || (has_full_house && joker_count >= 2) || (has_three_kind && joker_count == 2)
    {
        return EHandType::FiveKind;
    }
    if has_four_kind || joker_count == 3 || (has_three_kind && joker_count >= 1) || (has_two_pair && joker_count >= 2) {
        return EHandType::FourKind;
    }
    if has_full_house || (has_three_kind && joker_count == 1) || (has_two_pair && joker_count == 1) {
        return EHandType::FullHouse
    }
    if has_three_kind || (has_two_pair && joker_count >= 1) || (has_one_pair && joker_count >= 1){
        return EHandType::ThreeKind;
    }
    if has_two_pair || joker_count == 2 || (has_one_pair && joker_count == 1) {
        return EHandType::TwoPair;
    }
    if has_one_pair || joker_count == 1 {
